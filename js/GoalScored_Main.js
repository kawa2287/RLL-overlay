function GoalScoredMain(d) {
  ...
  $(".scorebug ." + side + " .overlay .g").css({
    "text-shadow": "2px 2px 6px " + colors.shadow,
  });

  //Flash Scoreboard Goal Animation
  FlashGoal(side);

  //Set Replay Transition Animation Logo
  $("#transitionLogo").css(
    "background-image",
    "url(" + TEAM_TRANSITION_MAP[teamName] + ")"
  );
  ...
  $(".replay .lastMan .name").css({
    "text-shadow": "2px 2px 8px " + oppColors.shadow,
  });
...
function GetGoalNetPosition(d, scoredOn) {
  ...
  console.log(
    "BALLX= " + ball.x + " BALLZ= " + ball.z + " SPEED = " + ball.speed
  );
  ...
  //set position
  $(".replay .goal .in b").css({
    transform: "translate(" + xTrans + "px," + zTrans + "px)",
  });
  $(".replay .goal .in i").css({
    transform: "translate(" + xTrans + "px," + zTrans + "px)",
  });
